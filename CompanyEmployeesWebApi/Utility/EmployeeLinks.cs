...
namespace CompanyEmployeesWebApi.Utility
{
    public class EmployeeLinks : IEmployeeLinks
    {
        ...
        private bool ShouldGenerateLinks(HttpContext httpContext)
        {
            ...
            return mediaType.SubTypeWithoutSuffix.EndsWith("hateoas",
                StringComparison.InvariantCultureIgnoreCase);//si termina con hateos devuelve true
        }
            ...
            string fields = "")
        {
            var links = new List<Link>
            {
                new Link(_linkGenerator.GetUriByAction
                    (httpContext, "GetEmployeeForCompany", values: new { companyId, id, fields }),
                    "self",
                    "GET"),
                new Link(_linkGenerator.GetUriByAction
