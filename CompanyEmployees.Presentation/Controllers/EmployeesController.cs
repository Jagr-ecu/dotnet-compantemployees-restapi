...
namespace CompanyEmployees.Presentation.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
            ...
            [FromQuery] EmployeeParameters employeeParameters)
        {
            ...
            var result = await _service.EmployeeService.GetEmployeesAsync(companyId, linkParams,
                trackChanges : false);
        ...
        {
            if (patchDoc is null)
                return BadRequest("El objeto patchDoc enviado desde el cliente es nulo");

            var result = await _service.EmployeeService.GetEmployeeForPatch(companyId, id,
                compTrackChanges: false, empTrackChanges: true);

            //La variable patchDoc solo se puede aplicar al tipo EmployeeForUpdateDto
